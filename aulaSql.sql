...
INSERT INTO venda(
data_venda, produto, categoria, valor_unitario, quantidade, cliente, cidade, estado)
VALUES(
'2025-09-15', 'Notebook Dell Inspiron', 'Informatica', 3500.00,1, 'Joao Silva', 'Rio de Janeiro', 'RJ');

INSERT INTO venda (data_venda, produto, categoria, valor_unitario, quantidade, cliente, cidade, estado)
VALUES
...
SUM(quantidade * valor_unitario) AS total_cliente
 ...
 GROUP BY cliente
 HAVING total_cliente > 5000 and total_cliente < 10000;
 
 SELECT cliente,
AVG(quantidade * valor_unitario) AS media_vendida
 FROM venda
 WHERE categoria IN ('Informatica', 'Acessórios')
 
 GROUP BY categoria

 #Tarefa 1: Liste os nomes dos produtos vendidos e seus respectivos valores unitários.
SELECT produto, valor_unitario
FROM venda
ORDER BY valor_unitario;

#Tarefa 2: Recupere todas as vendas realizadas na cidade de 'São Paulo’.
SELECT estado , COUNT(*) AS Quantidade_de_Venda
 FROM venda
 WHERE	estado = 'SP'
GROUP BY estado;

#Tarefa3: Calcule o valor total arrecadado com todas as vendas, considerando valor unitário x quantidade.
...
#Tarefa4: Agrupe os dados por categoria e calcule o total vendido em cada uma.
 SELECT categoria,
 SUM(valor_unitario * quantidade) AS Valor_total_Categoria
 FROM venda
 GROUP BY categoria
 ORDER BY categoria;
 
 #Tarefa5: Identifique os clientes que compraram mais de R$3000 em produtos.
 SELECT cliente, 
 SUM(valor_unitario * quantidade) AS Venda_Cliente
FROM venda
 GROUP BY cliente
 HAVING Venda_cliente > 3000;
 
 #Tarefa6: Liste todas as vendas realizadas na cidade de 'Curitiba’.
 SELECT cidade, COUNT(*) AS Quantidade_Venda_Curitiba
FROM venda
WHERE cidade = 'Curitiba'
group by cidade;

SELECT cidade, produto, valor_unitario AS Vendas_Curitiba
FROM venda
WHERE cidade = 'Curitiba'
GROUP BY cidade, produto
ORDER BY cidade, produto, valor_unitario;

#Tarefa7: Liste os clientes que acumularam mais de R$4000 em compras.
SELECT cliente,
SUM(valor_unitario * quantidade) AS Venda_4
FROM venda 
GROUP BY cliente
 HAVING Venda_4 > 4000;
 
 #Tarefa8: filtre clientes que compraram mais de R$5000. Compare o uso de WHERE e HAVING.
 SELECT cliente,
 SUM(valor_unitario * quantidade) AS Vendas_5
 WHERE cliente 
 #TERMINAR
 
 
 #TAREFA1: Quantas compras cada cliente realizou. Considere apenas vendas com valor unitário acima de R$200.
 SELECT cliente,
 SUM(valor_unitario * quantidade) AS Venda_200
 FROM venda
 GROUP BY cliente
 HAVING Venda_200 > 200;
 
 #TAREFA2: Agrupe as vendas por estado e calcule o total vendido. Considere apenas vendas realizadas nas cidades de Curitiba, Porto Alegre e Belo Horizonte. Exiba apenas os estados cujo total vendido ultrapasse R$5000
 SELECT estado, COUNT(*) AS Quantidade_Venda_Estados
 FROM venda
 WHERE estado = 'PR', 'RS', 'BH'
GROUP BY estado, produto
ORDER BY estado, produto, valor_unitario;
#TERMINAR

#TAREFA3: Calcule o valor médio gasto por cliente na categoria Informática. Exiba apenas os clientes cuja média de gasto seja superior a R$1000.
SELECT categoria,
